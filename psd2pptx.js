...
;(async () => {
    ...
    const argv = yargs
        ...
        .boolean("v").alias("v", "verbose").default("v", false).describe("v", "print verbose messages")
        .string("o").nargs("o", 1).alias("o", "output").default("o", "").describe("o", "output PPTX file")
        .version(false)
        .strict()
        .showHelpOnFail(true)
        .demand(1)
    ...
    let layersCanvas = layers.filter((item)       =>  item.path.match(/^Canvas\//i))
    let layersSlides = layers.filter((item)       => !item.path.match(/^Canvas\//i))
